...
func CreateEntry(ctx context.Context, ipfsInstance format.NodeAdder, identity *identityprovider.Identity, data *Entry, opts *iface.CreateEntryOptions) (iface.IPFSLogEntry, error) {
...
func CreateEntryWithIO(ctx context.Context, ipfsInstance format.NodeAdder, identity *identityprovider.Identity, data iface.IPFSLogEntry, opts *iface.CreateEntryOptions, io iface.IO) (iface.IPFSLogEntry, error) {
	if ipfsInstance == nil {
...
func (e *Entry) ToMultihash(ctx context.Context, ipfs format.NodeAdder, opts *iface.CreateEntryOptions) (cid.Cid, error) {
	io, err := cbor.IO(&Entry{}, &LamportClock{})
	if err != nil {
		return cid.Undef, err
	}

	return ToMultihashWithIO(ctx, e, ipfs, opts, io)
}

// ToMultihashWithIO gets the multihash of an Entry.
func ToMultihashWithIO(ctx context.Context, e iface.IPFSLogEntry, ipfs format.NodeAdder, opts *iface.CreateEntryOptions, io iface.IO) (cid.Cid, error) {
	if opts == nil {
	...
	return io.Write(ctx, ipfs, data, &iface.WriteOpts{
		Pin: opts.Pin,
	})
}
...
func FromMultihash(ctx context.Context, ipfs format.NodeGetter, hash cid.Cid, provider identityprovider.Interface) (iface.IPFSLogEntry, error) {
	io, err := cbor.IO(&Entry{}, &LamportClock{})
	if err != nil {
		return nil, err
	}
...
func FromMultihashWithIO(ctx context.Context, ipfs format.NodeGetter, hash cid.Cid, provider identityprovider.Interface, io iface.IO) (iface.IPFSLogEntry, error) {
	if ipfs == nil {
		return nil, errmsg.ErrIPFSNotDefined
	}
